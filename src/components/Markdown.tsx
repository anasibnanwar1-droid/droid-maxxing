...
const MermaidBlock = memo(function MermaidBlock({ code }: { code: string }) {
    ...
    return (
      <div className="rounded-2xl border border-droid-border/40 bg-droid-elevated/20 overflow-hidden my-5">
        ...
        <pre className="overflow-x-auto p-4">
          <code className="font-mono text-[12px] text-droid-text-secondary whitespace-pre">{code}</code>
        </pre>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-droid-border/40 bg-droid-elevated/20 overflow-hidden my-5">
      ...
      <div className="p-4 flex items-center justify-center [&_svg]:max-w-full [&_svg]:h-auto" dangerouslySetInnerHTML={{ __html: svg }} />
    </div>
  );
});
...
function HighlightJson({ code }: { code: string }) {
  const nodes = useMemo(() => {
    const tokens = code.split(/("(?:\\.|[^"\\])*"|:|true|false|null|\d+(?:\.\d+)?(?:[eE][+-]?\d+)?|[\[\]{}!,])/g);
    return tokens.map((token, i) => {
      if (token.match(/^"(?:\\.|[^"\\])*"$/)) {
        const next = tokens[i + 1]?.trimStart();
        if (next?.startsWith(':')) {
          return <span key={i} style={{ color: 'var(--droid-accent)' }}>{token}</span>;
        }
        return <span key={i} style={{ color: 'var(--droid-green)' }}>{token}</span>;
      }
      if (token === 'true' || token === 'false') return <span key={i} style={{ color: 'var(--droid-orange)' }}>{token}</span>;
      if (token === 'null') return <span key={i} style={{ color: 'var(--droid-text-muted)' }}>{token}</span>;
      if (token.match(/^\d/)) return <span key={i} style={{ color: 'var(--droid-orange)' }}>{token}</span>;
      if (/^[{}\[\],:!]$/.test(token)) return <span key={i} style={{ color: 'var(--droid-text-muted)' }}>{token}</span>;
...
function MarkdownImpl({ children, specMode }: { children: string; specMode?: boolean }) {
  return (
    <div className={`text-droid-text [overflow-wrap:anywhere] ${specMode ? 'text-[15px] leading-[1.8] space-y-5' : 'text-[13.5px] leading-[1.7] space-y-3'}`}>
      <ReactMarkdown
        ...
        components={{
          h1: ({ children }) => {
            ...
            return specMode ? (
              <h1 id={id} className="text-[24px] font-semibold tracking-tight text-droid-text mt-10 first:mt-0 mb-4 scroll-mt-8">
                {children}
              </h1>
            ) : (
              <h1 className="text-[17px] font-semibold text-droid-text mt-4 first:mt-0 mb-1">{children}</h1>
            );
          },
          h2: ({ children }) => {
            const text = String(children ?? '');
            const id = slugify(text);
            return specMode ? (
              <h2 id={id} className="text-[18px] font-semibold text-droid-text mt-7 first:mt-0 mb-2.5 scroll-mt-8">
                {children}
              </h2>
            ) : (
              <h2 className="text-[15px] font-semibold text-droid-text mt-4 first:mt-0 mb-1">{children}</h2>
            );
          },
          h3: ({ children }) => {
            const text = String(children ?? '');
            const id = slugify(text);
            return specMode ? (
              <h3 id={id} className="text-[15px] font-semibold text-droid-text-secondary mt-5 first:mt-0 mb-2 scroll-mt-8">
                {children}
              </h3>
            ) : (
              <h3 className="text-[14px] font-semibold text-droid-text mt-3 first:mt-0 mb-1">{children}</h3>
            );
          },
          p: ({ children }) => <p className={specMode ? 'leading-[1.8]' : 'leading-[1.7]'}>{children}</p>,
          ul: ({ children }) => <ul className={`marker:text-droid-text-muted ${specMode ? 'list-disc pl-6 space-y-2' : 'list-disc pl-5 space-y-1.5'}`}>{children}</ul>,
          ol: ({ children }) => <ol className={`marker:text-droid-text-muted ${specMode ? 'list-decimal pl-6 space-y-2' : 'list-decimal pl-5 space-y-1.5'}`}>{children}</ol>,
          li: ({ children }) => <li className={`${specMode ? 'leading-[1.75] pl-1' : 'leading-[1.65] pl-0.5'}`}>{children}</li>,
          ...
          code: ({ className, children }) => {
            ...
            if (inline)
              return <code className={`font-mono px-1.5 py-0.5 rounded-md bg-droid-elevated/70 text-droid-text [overflow-wrap:anywhere] ${specMode ? 'text-[13px]' : 'text-[12px]'}`}>{children}</code>;
            ...
            if (isJsonLang(className)) {
              return (
                <pre className={`rounded-xl bg-droid-elevated/40 overflow-x-auto ${specMode ? 'my-4 p-4' : 'my-2.5 p-3.5'}`}>
                  <code className={`font-mono leading-[1.65] whitespace-pre ${specMode ? 'text-[13px]' : 'text-[12px]'}`}>
            ...
            return (
              <pre className={`rounded-xl bg-droid-elevated/40 overflow-x-auto ${specMode ? 'my-4 p-4' : 'my-2.5 p-3.5'}`}>
                <code className={`font-mono leading-[1.65] text-droid-text-secondary whitespace-pre ${specMode ? 'text-[13px]' : 'text-[12px]'}`}>{children}</code>
              </pre>
            );
          },
          table: ({ children }) => (
