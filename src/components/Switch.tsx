...
export function Switch({ label, checked, onChange, disabled = false }: SwitchProps) {
  return (
    <button
      ...
      className={`w-10 h-6 rounded-full transition-colors shrink-0 flex items-center p-0.5 disabled:cursor-not-allowed disabled:opacity-60 ${checked ? 'bg-droid-accent' : 'bg-droid-border'}`}
    >
      <span
