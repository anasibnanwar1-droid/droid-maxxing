...
export type Provider = 'anthropic' | 'openai' | 'google' | 'factory' | 'meta' | 'mistral' | 'default';

export function providerOf(model?: ModelInfo, modelId?: string): Provider {
  const hay = `${model?.provider ?? ''} ${model?.displayName ?? ''} ${model?.id ?? modelId ?? ''}`.toLowerCase();
  ...
  if (/glm|kimi|deepseek|minimax|qwen|droid core|factory/.test(hay)) return 'factory';
  if (/llama|meta/.test(hay)) return 'meta';
  if (/mistral|mixtral/.test(hay)) return 'mistral';
  return 'default';
}

const MONOGRAM: Record<string, { label: string; color: string }> = {
  meta: { label: 'M', color: '#4267B2' },
  mistral: { label: 'M', color: '#ff7000' },
};

function FactoryMark({ size }: { size: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 67 65" fill="none" className="shrink-0 text-droid-text" aria-hidden>
      <path
        fill="currentColor"
...
export function ModelIcon({ provider, size = 18 }: { provider: Provider; size?: number }) {
  ...
  if (provider === 'anthropic')
    return (
      <svg {...s} viewBox="0 0 24 24" fill="none" className="shrink-0 text-droid-text" aria-hidden>
        <path fill="currentColor" d="m13.788825 3.932 6.43325 16.136075h3.5279L17.316725 3.932H13.788825Z" />
        <path fill="currentColor" d="m6.325375 13.682775 2.20125 -5.67065 2.201275 5.67065H6.325375ZM6.68225 3.932 0.25 20.068075h3.596525l1.3155 -3.3886h6.729425l1.315275 3.3886h3.59655L10.371 3.932H6.68225Z" />
      </svg>
    );
  ...
  const m = MONOGRAM[provider];
  if (!m) return <FactoryMark size={size} />;
  return (
    <span
      className="shrink-0 inline-flex items-center justify-center rounded-md font-semibold text-white"
      style={{ width: size, height: size, background: m.color, fontSize: size * 0.5 }}
    >
      {m.label}
    </span>
  );
