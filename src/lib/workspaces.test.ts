...
import { addWorkspaceCwd, buildWorkspaceSections, isSubagentSession, SIDEBAR_VISIBLE_SESSION_LIMIT } from './workspaces';
...
test('buildWorkspaceSections includes every known session for explicitly added workspaces', () => {
  const missions = [
    ...
    ...Array.from({ length: SIDEBAR_VISIBLE_SESSION_LIMIT + 2 }, (_, i) => mission(`repo-${i}`, '/repo/app', i + 1)),
  ];
...
test('buildWorkspaceSections can still cap an explicit bootstrap list', () => {
  const missions = Array.from({ length: SIDEBAR_VISIBLE_SESSION_LIMIT + 2 }, (_, i) => mission(`repo-${i}`, '/repo/app', i + 1));
...
test('isSubagentSession flags workers, validators and parented sessions', () => {
  ...
  assert.equal(isSubagentSession({ ...mission('v', '/repo/app', 1), kind: 'mission_validator' }), true);
  assert.equal(isSubagentSession({ ...mission('p', '/repo/app', 1), parentSessionId: 'parent' }), true);
});
